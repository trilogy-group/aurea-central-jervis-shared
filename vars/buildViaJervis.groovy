...
def call() {
    ...
    def git_service = new GitHub()
    git_service.gh_token = getGitHubAPIToken()
    def commit_msg = yaml.load(new URL("http://jervis-sandbox2.devfactory.com/job/trilogy-group/job/aurea-central-jervis-example/job/PR-5/59/api/json?pretty=true").newReader(requestProperties: ['Accept': 'application/json']))
